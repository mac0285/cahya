...
class M_penjualan extends CI_Model{
	...
	function simpan_penjualan($nofak,$total,$jml_uang,$kembalian){
		...
		$kode_dc=$this->session->userdata('kode_dc');
		$this->db->query("INSERT INTO tbl_jual (jual_nofak,jual_total,jual_jml_uang,jual_kembalian,jual_user_id,jual_keterangan,kode_dc) VALUES ('$nofak','$total','$jml_uang','$kembalian','$idadmin','TRS','$kode_dc')");
	...
	function simpan_penjualan_grosir($nofak,$total,$jml_uang,$kembalian){
		$idadmin=$this->session->userdata('idadmin');
		$kode_dc=$this->session->userdata('kode_dc');
		$this->db->query("INSERT INTO tbl_jual (jual_nofak,jual_total,jual_jml_uang,jual_kembalian,jual_user_id,jual_keterangan,kode_dc) VALUES ('$nofak','$total','$jml_uang','$kembalian','$idadmin','grosir','$kode_dc')");
		foreach ($this->cart->contents() as $item) {
			$data=array(
				'd_jual_nofak' 			=>	$nofak,
				'd_jual_barang_id'		=>	$item['id'],
